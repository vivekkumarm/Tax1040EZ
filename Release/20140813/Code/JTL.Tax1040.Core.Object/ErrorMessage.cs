...
namespace JTL.Tax1040.Core.Object
{
    ...
    public class EfileErrorCollection : List<ErrorMessage>, IEnumerable<SqlDataRecord>
    {
        IEnumerator<SqlDataRecord> IEnumerable<SqlDataRecord>.GetEnumerator()
        {

            var sdr = new SqlDataRecord(
             new SqlMetaData("errorcode", SqlDbType.VarChar, 50)
            );

            foreach (ErrorMessage errormsg in this)
            {
                sdr.SetSqlString(0, errormsg.ErrorCode);
                yield return sdr;
            }
        }
    }
