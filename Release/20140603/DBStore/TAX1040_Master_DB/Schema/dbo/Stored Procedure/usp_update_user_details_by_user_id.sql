...
BEGIN  
IF   EXISTS(SELECT 1 FROM [dbo].[user] WITH(NOLOCK)WHERE [user_id] != @user_id AND [email] = @email AND  [is_active] = 1  AND [is_deleted] = 0)                                               
   BEGIN   
	  BEGIN  
         SELECT  @email_exist	      
      END  
...
IF  EXISTS(SELECT 1 FROM [dbo].[user] WITH(NOLOCK)WHERE  [user_id] != @user_id AND [username] = @username AND [is_active] = 1  AND [is_deleted] = 0)                                               
   BEGIN   
     BEGIN  
...
ELSE   
    ...
    END      
     ...
     IF(@security_answer != '' AND @salt != '' AND @security_question_id != 0)  
    BEGIN  
        UPDATE [dbo].[user]   
      SET  
