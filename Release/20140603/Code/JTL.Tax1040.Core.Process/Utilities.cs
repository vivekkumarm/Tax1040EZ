...
namespace JTL.Tax1040.Core.Process
{
    public static class Utilities
    {
        ...
        public static bool IsValidPhone(string value)
        {
            ...
            {
                string phonenumber = RemoveCloseBracket(value);
                phonenumber = RemoveOpenBracket(phonenumber);
                phonenumber = RemoveHiphen(phonenumber);
                phonenumber = RemoveUnderscore(phonenumber);
        ...
        public static bool IsValidSSN(string value)
        {
            if (!string.IsNullOrEmpty(value))
            {
                ...
                if (IsNumeric(ssn))
                {
                    ...
                    if (ssn == Constants.NineZeros || ssn == Constants.NineNines || ssn == Constants.NaturalNumbers || ssn.Length != Constants.SSNLength || areaCode == Constants.InvalidSsnAreaCode || areaCode > Constants.InvalidSsnAreaCodeRange)
                    {
                        return false;
                    }
                    else
