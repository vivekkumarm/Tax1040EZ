...
$(document).ready(function () {
    ...
    $('#Address').validate({
        rules: {
            dtFromDate:
                {
                    validateDate: function () { $('#divFromDateErrorMsg').html(''); true; },
                },
            dtToDate:
                {
                    validateDate: function () { $('#divToDateErrorMsg').html(''); true; },
                },
            txtZipCode:
                {
                    minlength: 5,
        ...
        messages: {
            ...
            dtFromDate:
                 {
                     validateDate: function () { $('#divFromDateErrorMsg').html('You have entered an invalid date'); }
                 },
            dtToDate:
                {
                    validateDate: function () { $('#divToDateErrorMsg').html('You have entered an invalid date'); }
                },
            txtZipCode:
                {
    ...
    $("#chkEarnedIncomeSateYouNotLived").change(function () {
        //$("#divEarnedIncomeState").toggle();
        if ($('#chkEarnedIncomeSateYouNotLived').prop('checked')) {
            $('#divEarnedIncomeState').show();
        }
        else {
            $('#divEarnedIncomeState').hide();
...
function validateDateAndState() {
    ...
    if (fromDate) {
        $("#divFromDateError").hide();
    }
    else {
        $("#divFromDateError").show();
        $("#divdtToDateError").html('New State of Residence From Dateshould be within Tax year');
        erorcount = erorcount + 1;
    }

    if (toDate) {
        $("#divdtToDateError").hide();
    }
    else {
        $("#divdtToDateError").show();
        $("#divdtToDateError").html('New State of Residence To Date should be within Tax year');
        erorcount = erorcount + 1;
    }
    ...
    if (fromDate && toDate && $("#dtFromDate").val() > $("#dtToDate").val()) {
        $("#divFromDateError").show();
        $("#divFromDateError").html("You have entered an Invalid Date. Check your To date for New state of Residence.");
        erorcount = erorcount + 1;
    }
    else {
        $("#divFromDateError").hide();
...
function PersistAddress() {
    ...
    if ($("#Address").valid() && validateDateAndState()) {
        var addressType;
