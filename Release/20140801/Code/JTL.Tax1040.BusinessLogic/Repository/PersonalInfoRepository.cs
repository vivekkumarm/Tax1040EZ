    ...
    public class PersonalInfoRepository : IPersonalInfoRepository
    {
                                       ...
                                       ErrorMessages errorMessages)
        {
                ...
                personalDetails.PrimaryTaxPayer.Person.Address != null) // 09Jul2014 Sathsih checked personal address not null
            {
                ...
                {
                    //Have to remove the below regions once implemented the list

                    //#region LivedInAnotherState
                         ...
                         && personalDetails.PrimaryTaxPayer.Person.Address.LivedInAnotherStates.Any(st => st.StateId != 0))
                    {
                        ...
                        {
                            //For Future Referance to call a index value
                            //var indexValue = personalDetails.PrimaryTaxPayer.Person.Address.LivedInAnotherStates.FindIndex(st=>st.StateId==personalDetails.PrimaryTaxPayer.Person.Address.ResidenceState.StateId);
                           errorMessage =  errorMessages[Constants.TAXPAYER_ADDRESS_SAMERESIDENT_NAME];

                        if (errorMessage != null)
                        {
                            //For Future Referance to call a index value check
                            //if(indexValue> -1)
                        ...
                        && personalDetails.PrimaryTaxPayer.Person.Address.EarnedIncomeFromNonLivedStates.All(st => st.StateId != 0)
                        && personalDetails.PrimaryTaxPayer.Person.Address.LivedInAnotherStates.Any(st => personalDetails.PrimaryTaxPayer.Person.Address.EarnedIncomeFromNonLivedStates.Contains(st))
                          )
                        {
                            //TODO : Have to change the error message once spec is updated
                            errorMessage = errorMessages[Constants.TAXPAYER_ADDRESS_SAMERESIDENT_NAME];
                            if (errorMessage != null)
                                errorMessageList.Add(errorMessage);
                        }
                ...
                if (personalDetails.PrimaryTaxPayer.Person.Address.HasEarnedIncomeFromNonLivedState.HasValue && personalDetails.PrimaryTaxPayer.Person.Address.HasEarnedIncomeFromNonLivedState.Value)
                {
                    ...
                    //#region EarnedIncomeFromNonLivedState
                    //errorMessage =
                    //    BusinessValidationRepository.AssertIsNotNull(
