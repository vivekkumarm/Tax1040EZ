        ...
        public long PersitEarnedIncomeCredit(long userId, TaxReturnData taxReturnData)
        {
            try
            {
                ...
                if (eic != null)
                {
                    ...
                    var outputGetData = Microsoft.Activities.Extensions.WorkflowArguments.FromDictionary(WorkflowInvoker.Invoke(new F1040EZAdjustedGrossIncome(), input));
        ...
        public dynamic GetCreditSummary(UserDTO userDTO)
        {
            try
            {
                ...
                if (taxObject != null)
                {
                    ...
                    creditSummaryData.ErrorMessages = taxObject.ErrorMessages.Where(err => err.Topic == Constants.TOPIC_EARNED_INCOME_CREDIT && err.ErrorCode.StartsWith("EarnedIncomeCredit_RF")||err.ErrorType.StartsWith("Warning"));
