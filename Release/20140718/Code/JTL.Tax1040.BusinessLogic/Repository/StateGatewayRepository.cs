...
namespace JTL.Tax1040.BusinessLogic
{
    public class StateGatewayRepository : IStateGatewayRepository
    {
        ...
        public void PersisttaxableState(List<FilingState> taxableState, long userId, long userDataId)
        {
            if (taxableState != null)
            {
                ...
                if (taxObject.StateBO.FilingStates == null)
                {
                    taxObject.StateBO.FilingStates = new List<FilingState>();
                }

                FilingState state = taxableState.Where(a => a.ResidentType == ResidentType.Resident.ToString()).FirstOrDefault();
                ...
                if (state != null && taxableState.Count > 0 && !Utilities.IsStringEmpty(state.ResidentType))
                {
                    ...
                    if (state.State != null)
                    {
                        taxObject.PersonalDetails.PrimaryTaxPayer.Person.Address.ResidenceState.StateId = state.State.StateId;
                        taxObject.PersonalDetails.PrimaryTaxPayer.Person.Address.ResidenceState.StateName = state.State.StateName;

                    }
                    taxableState.Remove(state);
                }

                taxObject.StateBO.FilingStates = taxableState;
        ...
        public dynamic GetAddressByUserIdAndUserDataId(long userId, long userDataId)
        {
            ...
            try
            {
                ...
                personalDetails.Taxablestate = taxObject.StateBO.FilingStates;
