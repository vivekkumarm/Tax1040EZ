...
var taxAndWrapupErrorCount = 0;
var creditsErrorCountleftmenu = 0;
var containingFolder = "";
var currentPersonalSection;
...
document.write("<div class='row crumb'>" +
    ...
    "<a id='aTaxesAndWrapUp' href='../TaxesAndWrapUp/taxesandwrapupchecklist.html' class='btn btn-default'>Taxes & Wrap Up<sup id='supTaxesWrapUpErrorTypeIcon' style='display:none;' data-toggle='tooltip'  data-placement='top' title='' ></sup></a>" +
     "<a id='aState' href='../StateGateway/stategateway.html' class='btn btn-default'>State<sup id='supStateErrorTypeIcon' style='display:none;'  data-toggle='tooltip'  data-placement='top' title='' ></sup></a>" +
...
var sessionPollingInterval = 60000;
var sessionExpirationMinutes = 20;
var sessionWarningMinutes = 18;
...
function sessionSetInterval() {
    sessionIntervalID = setInterval('sessionInterval()', sessionPollingInterval);
}
...
function sessionLogOut() {
    ...
    window.location.href = '../FinalPage/timeout.html';
}
function sessionInterval() {
    var now = new Date();
    //get milliseconds of differneces
    var timediff = now - sessionLastActivity;
    //get minutes between differences
    var diffMins = (timediff / 1000 / 60);
    if (diffMins >= sessionWarningMinutes) {
        //wran before expiring
        //stop the timer
        sessClearInterval();
        //promt for attention
        var active = confirm('Your session will expire in ' + (sessionExpirationMinutes - sessionWarningMinutes) +
            ' minutes (as of ' + now.toTimeString() + '), press OK to remain logged in ' +
            'or press Cancel to log off. \nIf you are logged off any changes will be lost.');
        if (active == true) {
            now = new Date();
            timediff = now - sessionLastActivity;
...
$(document).ready(function () {
    if (window.sessionStorage.getItem("disable")) {
        ...
        $('#aEfile').removeClass('active').addClass('disabled', true);
        window.sessionStorage.removeItem("disable")
    }
    containingFolder = window.location.pathname.split('/')[1];

    //06Aug14, Bala code grouped as function to call the function from various place
    getErrorCountAndDisableBreadcrum();

    GetCurrentPersonalSection();
    
    InitCurrentSession();
});


function getErrorCountAndDisableBreadcrum() {
        ...
        success: function (data, textStatus) {
            if (data != null && data.m_Item1 != null) {
                $.each(data.m_Item1, function (index, value) {
                    ...
                    //20Aug2014-Thileep Modified for PageInfo Error type
                    if (value.Tab == Tab.Credits) {
                        ...
                        else if (value.ErrorType == ErrorType.Information || value.ErrorType == ErrorType.PageInfo) {
            ...
            if (data != null && data.m_Item3 != null) {
                ...
                personalInfoEligibilityErrorCount = 0;
                incomeEligibilityErrorCount = 0;
                deductionEligibilityErrorCount = 0;
                creditsEligibilityErrorCount = 0;
                taxesAndWrapUpEligibilityErrorCount = 0;
                stateEligibilityErrorCount = 0;
            }
            if (personalInfoEligibilityErrorCount > 0) {
                $('#aIncome').attr('disabled', true).addClass('tip');
                $('#aIncome').attr('tabIndex', -1);

                $('#aDeduction').attr('disabled', true);
                $('#aDeduction').attr('tabIndex', -1);

                $('#aCredits').attr('disabled', true);
                $('#aCredits').attr('tabIndex', -1);

                $('#aTaxesAndWrapUp').attr('disabled', true);
                $('#aTaxesAndWrapUp').attr('tabIndex', -1);

                $('#aReview').attr('disabled', true);
                $('#aReview').attr('tabIndex', -1);

                $('#aEfile').attr('disabled', true);
                $('#aEfile').attr('tabIndex', -1);

                $('#aState').attr('disabled', true);
                $('#aState').attr('tabIndex', -1);
                ...
                $('#aIncome').attr('disabled', false);
                $('#aDeduction').attr('disabled', false);
                $('#aCredits').attr('disabled', false);
                $('#aTaxesAndWrapUp').attr('disabled', false);
                $('#aState').attr('disabled', false);
                $('#aReview').attr('disabled', false);
                $('#aEfile').attr('disabled', false);

                if (incomeEligibilityErrorCount > 0) {
                    $('#aDeduction').attr('disabled', true);
                    $('#aDeduction').attr('tabIndex', -1);
            ...
            //vivek 4-Aug-14 error message count for the tax and wrapup
            if (data.m_Item5.m_Rest.m_Item1 > 0) {
                taxAndWrapupErrorCount = data.m_Item5.m_Rest.m_Item1;
            }
            //vivek 4-Aug-14 error message count for the EIC

            if (data.m_Item5.m_Rest.m_Item2 > 0) {
                creditsErrorCountleftmenu = data.m_Item5.m_Rest.m_Item2;
            }
...
//07Aug2014 Sathish get personal current session to show error count in left menu
function GetCurrentPersonalSection() {
    $.ajax({
        url: serviceUrl + 'personalinfo/getcurrentpersonalsection',
        async: false,
        dataType: "json",
        type: "POST",
        beforeSend: function (request) {
