...
//$(function () {
//    $("#popUserName").popover({ title: '', content: "<h5>What is your Username?</h5>This is how we identify and address you at Tax Garden.You might have chosen to use your Email Address as your Username.<br /><a href='' target='_blank' class='more'>Read Common Question</a>" });
    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            if (data.UserId == 0) {
                ...
                $("#divSignInError").html("The Username or Password  you entered is Incorrect. Try again!");
                ...
                //Sathish.N 05Aug2014 Delete efile information while user log in. As discussed with praveen every time user login IRS Transmission detail will be cleared 
                DeleteIRSTransmissionDetail();
                //setTimeout(function () {
...
//Sathish.N 05Aug2014 Delete efile information while user log in. As discussed with praveen every time user login IRS Transmission detail will be cleared
function DeleteIRSTransmissionDetail() {
    if (window.sessionStorage.getItem("UserDataId") != 0
        && window.sessionStorage.getItem("UserId") != 0) {
        $.ajax({
            url: serviceUrl + 'efile/deleteirstransmissiondetail',
            async: false,
            dataType: "json",
            type: "POST",
            beforeSend: function (request) {
