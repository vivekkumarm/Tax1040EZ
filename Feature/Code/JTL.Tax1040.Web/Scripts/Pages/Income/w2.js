...
//22 Aug 2014 - Vincent  - Defect # 14524 - Removed unwanted Key up check for household wages
...
var parameters;
//Yogalakshmi - 27th August 2014 - Changed Enum 27 to empty string
var EMPTY_STRING_ENUM_VALUE = "";

$(document).ready(function () {
    //Added by Praveen T on 29 May 2014
    $('.Title').html(TITLE);
    $('#radYes').removeClass("form-control tip amountField").addClass("form-radio");
   ...
   parameters = pageURL.split('&');
    ...
    if (parameters.length == 2 || parameters.length == 3) {
        ...
        if (controls.length == 2) {
            divToExpand = controls[0];
            controlToFocus = controls[1];
        }
    }
    ...
    $("#chkW2EmployerAddress").click(function () {
        ...
        if ($('#chkW2EmployerAddress').prop('checked') == false) {
            ...
            $("#divEmployee").hide();
            $('#aEmployeeAddress').removeClass('error'); //Saravanan N - 19th Aug, 2014 - Remove error icon if user uncheck the checkbox.
        }
    ...
    $('.validateControl3').blur(function () {
        ValidateWagesThirdAccordion();
    });

    //Saravanan N - 1st Sep, 2014 - For each text box in FOURTH accordion onblur event added to check whether error icon need to display in this accordion.
    $('#collapseFour input').each(function () {
        $(this).addClass('validateControl4');
        ...
        //}
        //else {
        //    $(this).removeClass('validateControl4');
        //}
    })

    $('.validateControl4').blur(function () {
        ValidateWagesFourthAccordion();
    });


    //Saravanan N - 1st Sep, 2014 - For each text box in FIFTH accordion onblur event added to check whether error icon need to display in this accordion.
    $('#collapseFive input').each(function () {
        $(this).addClass('validateControl5');
        ...
        //}
        //else {
        //    $(this).removeClass('validateControl5');
        //}
    })

    $('.validateControl5').blur(function () {
        ValidateWagesFifthAccordion();
    });


    //Saravanan N - 1st Sep, 2014 - For each text box in HOUSE HOLD WAGES accordion onblur event added to check whether error icon need to display in this accordion.
    $('#collapseSeven input').each(function () {
        //Add class only if it is having 'error' class (i.e., having red border)            
        $(this).addClass('validateControl7');
    })

    $('.validateControl7').blur(function () {
        ValidateHouseholdAccordion();
    });


    //Saravanan N - 1st Sep, 2014 - For each text box in SCHOLARSHIP INSTITUTION accordion onblur event added to check whether error icon need to display in this accordion.
    $('#collapseEight input').each(function () {
        $(this).addClass('validateControl8');
        ...
        //}
        //else {
        //    $(this).removeClass('validateControl8');
        //}
    })

    $('.validateControl8').blur(function () {
        ValidateScholarshipInsitutionAccordion();
    });


    //Saravanan N - 19th Aug, 2014 - Validate scholarship institution and house hold employee accordions during page load.
    ValidateWagesFirstAccordion();
    ValidateWagesSecondAccordion();
    ValidateWagesThirdAccordion();
    ValidateWagesFourthAccordion();
    ...
    if (divToExpand != null && controlToFocus != null) {
        //Saravanan N - 1st Sep, 2014 - Set first control focus while coming from Income Summary Diagnostics page.
...
function ClientSideValidationForFirstAndSectondAccordion() {
    if (($('#txtW2EmployerID').val().length >= 1 && $('#txtW2EmployeeZip').val().length < 5) || ($('#txtW2EmployerCity').val().length >= 1 &&
$('#txtW2EmployerCity').val().length < 5)) {
        $('#aEmployerInfo').addClass('error');
    }
    else {
        $('#aEmployerInfo').removeClass('error');
    }

    //If second accordion is having error, it should display error icon now.
    if (($('#txtW2EmployeeZip').val().length >= 1 && $('#txtW2EmployeeZip').val().length < 5) || ($('#txtW2EmployeeCity').val().length >= 1 &&
        $('#txtW2EmployeeCity').val().length < 5)) {
        $('#aEmployeeAddress').addClass('error');
    }
    else {
        $('#aEmployeeAddress').removeClass('error');
    }

}

function SetAccordionEvents() {
    //Saravanan N - 18th Aug, 2014 - While Expand the accordion display the client side errors.
    //TODO: Below two methods Commented by Saravanan - Once tested it can be removed.
...
function SetControlAcceptableFormat() {
...
function SetClientSideValidations() {
    $("#W2").validate({
        rules: {
            txtW2FederalWages: {
                required: true,
...
function BindEvents() {
    ...
    $('#btnchromeenter').click(function () {
        ...
        else if ($('#collapseFive').attr('class') == 'panel-collapse collapse in') {
            $('#btnContinue').click();
            return false;
        }
    });
    ...
    $('#btnSubmitOne').click(function () {
        //if (!$("#W2").valid()) {
        //    ////TODO: If exceptions occurred. Display error icon in the Accordion heading. - Saravanan N - 18th Aug, 2014.
        //}

        //Saravanan N - 18th Aug, 2014 - As uma suggested, eventhough user enter invalid date he should not stop at any places.
        ...
        //While click Next button, accordion hidden.bs event didn't fire. Hence, manually check second accordions validation error to display icon - Saravanan N - 27th Aug, 2014.
        ValidateWagesSecondAccordion();
        return false;
    });
...
function ValidateWagesFirstAccordion() {
...
function GetFirstAccordionClientSideValidationStatus() {
    ...
    //if Server side required entry added 'error' class for the control, then no need to validate the same control for client side validation to add error icon for the accordion. - Saravanan N - 1st Sep, 2014.
    var isEmployerZipValid = ($('#txtW2EmployerID').hasClass('error') ? true : $('#txtW2EmployerID').valid());
    var isEmployerCityValid = ($('#txtW2EmployerCity').hasClass('error') ? true : $('#txtW2EmployerCity').valid());
    var isEmployerZipValid = ($('#txtW2EmployerZip').hasClass('error') ? true :  $('#txtW2EmployerZip').valid());

    if (!isEmployerZipValid || !isEmployerCityValid || !isEmployerZipValid) {
...
//Saravanan N - 19th Aug, 2014 - Added method to validate Employee Address (second) accordion.
function ValidateWagesSecondAccordion() {
    var isClientSideValidationFailed = GetSecondAccordionClientSideValidationStatus();
...
function GetSecondAccordionClientSideValidationStatus() {
    var isValidationFailed = false;

    //if Server side required entry added 'error' class for the control, then no need to validate the same control for client side validation to add error icon for the accordion. - Saravanan N - 1st Sep, 2014.
    var isEmployeeCityValid = ($('#txtW2EmployeeCity').hasClass('error') ? true : $('#txtW2EmployeeCity').valid());
    var isEmployeeZipValid = ($('#txtW2EmployeeZip').hasClass('error') ? true : $('#txtW2EmployeeZip').valid());

    if (!isEmployeeCityValid || !isEmployeeZipValid) {
        isValidationFailed = true;
    }
...
function GetSecondAccordionServerSideValidationStatus() {
    var isValidationFailed = false;
    $('#collapseTwo input:text').each(function () {
        if ($(this).hasClass('error')) {
            isValidationFailed = true;
        }
    });
...
//------------------------------------------------------------- FOURTH ACCORDION VALIDATION -----------------------------------------------------------//
function ValidateWagesFourthAccordion() {
    var isServerSideValidationFailed = GetFourthAccordionServerSideValidationStatus();

    if (isServerSideValidationFailed) {
        $('#aW2Benefits').addClass('error');
    }
    else {
        $('#aW2Benefits').removeClass('error');
    }
}

function GetFourthAccordionServerSideValidationStatus() {
    var isValidationFailed = false;
    $('#collapseFour input:text').each(function () {
        if ($(this).hasClass('error')) {
            isValidationFailed = true;
        }
    });
...
//------------------------------------------------------------- FIFTH ACCORDION VALIDATION -----------------------------------------------------------//
function ValidateWagesFifthAccordion() {
    var isServerSideValidationFailed = GetFifthAccordionServerSideValidationStatus();

    if (isServerSideValidationFailed) {
        $('#aStateandLocalInfo').addClass('error');
    }
    else {
        $('#aStateandLocalInfo').removeClass('error');
    }
}

function GetFifthAccordionServerSideValidationStatus() {
    var isValidationFailed = false;
    $('#collapseFive input:text').each(function () {
        if ($(this).hasClass('error')) {
            isValidationFailed = true;
        }
    });
...
//------------------------------------------------------------- SEVENTH (HOUSE HOLD WAGES) ACCORDION VALIDATION ----------------------------------------//

//Saravanan N - 19th Aug, 2014 - Added method to validate household accordion.
function ValidateHouseholdAccordion() {
    ...
    //If all the first accordion control client side validation pass then remove the error class orElse Add. - Saravanan N - 27th Aug, 2014.
    if (isClientSideValidationFailed || isServerSideValidationFailed) {
        $('#aHouseHold').addClass('error');
    }
    else {
        $('#aHouseHold').removeClass('error');
    }
}

function GetHouseHoldWagesAccordionClientSideValidationStatus() {
    var isValidationFailed = false;
    var isEmployerCityValid =  ($('#txtHouseholdEmployerCity').hasClass('error') ? true : $('#txtHouseholdEmployerCity').valid());
    var isEmployerZipValid = ($('#txtHouseholdEmployerZip').hasClass('error') ? true :  $('#txtHouseholdEmployerZip').valid());

    if (!isEmployerCityValid || !isEmployerZipValid) {
        isValidationFailed = true;
    }
...
function GetHouseHoldWagesAccordionServerSideValidationStatus() {
    var isValidationFailed = false;
    $('#collapseSeven input:text').each(function () {
        if ($(this).hasClass('error')) {
            isValidationFailed = true;
        }
    });
...
//------------------------------------------------------------- EIGHTH (SCHOLARSHIP INSTITUTION) ACCORDION VALIDATION -------------------------------//
//Saravanan N - 19th Aug, 2014 - Added method to validate scholarship institution accordion.
function ValidateScholarshipInsitutionAccordion() {
    var isClientSideValidationFailed = GetScholarshipInsitutionAccordionClientSideValidationStatus();
    var isServerSideValidationFailed = GetScholarshipInstitutionAccordionServerSideValidationStatus();

    //If all the first accordion control client side validation pass then remove the error class orElse Add. - Saravanan N - 27th Aug, 2014.
    if (isClientSideValidationFailed || isServerSideValidationFailed) {
        $('#aScholarshipInstitution').addClass('error');
    }
    else {
        $('#aScholarshipInstitution').removeClass('error');
    }
}

function GetScholarshipInsitutionAccordionClientSideValidationStatus() {
    var isValidationFailed = false;
    var isInstitutionCityValid = ($('#txtInstitutionCity').hasClass('error') ? true : $('#txtInstitutionCity').valid());
    var isInstitutionZipValid = ($('#txtInstitutionZip').hasClass('error') ? true : $('#txtInstitutionZip').valid());

    if (!isInstitutionCityValid || !isInstitutionZipValid) {
        isValidationFailed = true;
    }
...
function GetScholarshipInstitutionAccordionServerSideValidationStatus() {
    var isValidationFailed = false;
    $('#collapseEight input:text').each(function () {
        if ($(this).hasClass('error')) {
            isValidationFailed = true;
        }
    });
...
function PersistW2() {
    ...
    $.each($('#divLocalTaxInfo').children('div'), function (key, value) {
        ...
        localTax = {
           ...
           // StateCode: (($("#ddlW2AssociatedState" + rowIndex + " option:selected").val() != 0) ? $("#ddlW2AssociatedState" + rowIndex + " option:selected").text() : ''),
    ...
    $.each($('#divLocalStateWagesInfo').children('div'), function (key, value) {
        ...
        stateLocalWage = {
            State: {
                ...
                StateCode: (($("#ddlW2Box15State" + rowIndex + " option:selected").val()!=0) ? $("#ddlW2Box15State" + rowIndex + " option:selected").text() : ''),
            },
            ...
            //Yogalakshmi - 27th August 2014 - StateIdNumber is in StateLocalTaxInfo class
            StateIdNumber: $("#txtW2StateID" + rowIndex).val().trim().length == 0 ? null : $("#txtW2StateID" + rowIndex).val(),//Saravanan N - 21st Aug, 2014 - StateIdNumber used in place of StateCode.
    ...
    if (employmentType > 0 && employmentType != 6 && employmentType != 7 && employmentType != 8) {
        ...
        stateCode = $("#ddlW2State option:selected").text();
        ...
        stateCode = $("#ddlHouseholdEmployerState option:selected").text();
        ...
        stateCode = $("#ddlInstitutionState option:selected").text();
    }
    ...
    var w2Data = {
        ...
        //Yogalakshmi - 19th August 2014 - Removed the method GetZeroOnIsNullOrEmpty() as NonQualifiedPlansAmount is a nullable field.
        NonQualifiedPlansAmount: ($.isNumeric($("#txtQualifiedPlan").val().trim()) ? $("#txtQualifiedPlan").val().trim() : null),
            ...
            State:
                {
                    ...
                    StateCode: (($("#ddlW2EmployeeState").val() != 0) ? $("#ddlW2EmployeeState option:selected").text() : '') //18Aug2014 Sathish pased empty string if no option selected for pdf binding
                },
        ...
        //Yogalakshmi - 19th August 2014 - EmployerCode checked for null using a new Enum Empty(EMPTY_STRING_ENUM_VALUE).
        W2EmployerCodes: [
            {
                EmployerCode: $("#ddlW212A").val() == 0 ? null : $("#ddlW212A").val(),
                Amount: ($.isNumeric($("#txt12AAmount").val().trim()) ? $("#txt12AAmount").val().trim() : null),
            },
            {
                EmployerCode: $("#ddlW212B").val() == 0 ? null : $("#ddlW212B").val(),
                Amount: ($.isNumeric($("#txt12BAmount").val().trim()) ? $("#txt12BAmount").val().trim() : null),
            },
            {
                EmployerCode: $("#ddlW212C").val() == 0 ? null : $("#ddlW212C").val(),
                Amount: ($.isNumeric($("#txt12CAmount").val().trim()) ? $("#txt12CAmount").val().trim() : null),
                Index: 2
            },
            {
                EmployerCode: $("#ddlW212D").val() == 0 ? null : $("#ddlW212D").val(),
            ...
            Organization: {
                ...
                Address: {
                    CompanyAddress: {
                        ...
                        State: {
                            StateId: stateId,
                            StateCode: ((stateId != 0) ? stateCode : '')  //18Aug2014 Sathish pased empty string if no option selected for pdf binding
                        }
                    }
                }
            }
                ...
                if (data.m_Item1.EmploymentType > 0 && data.m_Item1.EmploymentType != 6 && data.m_Item1.EmploymentType != 7) {
                    ...
                    if (data.m_Item1.W2EmployerCodes != null) {
                        $.each(data.m_Item1.W2EmployerCodes, function (index, value) {
                            ...
                            if (value != null) {
                                if (index == 0) {
                                    //Yogalakshmi - 19th August 2014 - If the EmployerCode is Empty(EMPTY_STRING_ENUM_VALUE) then changing it to NoneSelected(0).
                                    //Yogalakshmi - 27th August 2014 - Changed Enum 27 to null
                                    $("#ddlW212A").val(value.EmployerCode == null ? 0 : value.EmployerCode);
                                    $("#txt12AAmount").val(value.Amount == null ? "" : value.Amount.toFixed(2));
                                }
                                if (index == 1) {
                                    $("#ddlW212B").val(value.EmployerCode == null ? 0 : value.EmployerCode);
                                    $("#txt12BAmount").val(value.Amount == null ? "" : value.Amount.toFixed(2));
                                }
                                if (index == 2) {
                                    $("#ddlW212C").val(value.EmployerCode == null ? 0 : value.EmployerCode);
                                    $("#txt12CAmount").val(value.Amount == null ? "" : value.Amount.toFixed(2));
                                }
                                if (index == 3) {
                                    $("#ddlW212D").val(value.EmployerCode == null ? 0 : value.EmployerCode);
                                    $("#txt12DAmount").val(value.Amount == null ? "" : value.Amount.toFixed(2));
                                }
                            }
                        });
                    }
                    ...
                    if (data.m_Item1.OtherDeductsBenefits != null) {
                        $.each(data.m_Item1.OtherDeductsBenefits, function (index, value) {
                            ...
                            $("#txtW2Amount" + otherDeductionBenefitsRecordCount).val(value.Amount == null ? "" : value.Amount.toFixed(2));
                        });
                    }
                        ...
                        $.each(data.m_Item1.W2StateLocalTaxInfo, function (index, value) {
                            ...
                            if (data.m_Item1.W2StateLocalTaxInfo[stateWagesRecordCount].W2LocalTaxInfo != null) {
                                $.each(data.m_Item1.W2StateLocalTaxInfo[stateWagesRecordCount].W2LocalTaxInfo, function (index, value) {
                                    ...
                                    $("#txtW2LocalWages" + localTaxInfoRecordCount).val(value.LocalWagesAmount == null ? "" : value.LocalWagesAmount.toFixed(2));
                                    $("#txtW2LocalWithHeld" + localTaxInfoRecordCount).val(value.LocalTaxWithholdingAmount == null ? "" : value.LocalTaxWithholdingAmount.toFixed(2));
                            ...
                            $("#txtW2StateID" + stateWagesRecordCount).val(value.StateIdNumber); //Saravanan N - 21st Aug, 2014 - StateIdNumber used in place of StateCode.
                            $("#txtW2StateWages" + stateWagesRecordCount).val(value.StateWagesAmount == null ? "" : value.StateWagesAmount.toFixed(2));
                            $("#txtW2StateWithheld" + stateWagesRecordCount).val(value.StateTaxWithholdingAmount == null ? "" : value.StateTaxWithholdingAmount.toFixed(2));
                        });
                    }

                    $("#txtW2FederalWages").val(data.m_Item1.WagesAmount == null ? "" : data.m_Item1.WagesAmount.toFixed(2)),
                    $("#txtW2FederalWithheld").val(data.m_Item1.FederalTaxWithholdingAmount == null ? "" : data.m_Item1.FederalTaxWithholdingAmount.toFixed(2)),
                    $("#txtW2SSWages").val(data.m_Item1.SocialSecurityWagesAmount == null ? "" : data.m_Item1.SocialSecurityWagesAmount.toFixed(2)),
                    $("#txtW2SSwithheld").val(data.m_Item1.SocialSecurityTaxWithholdingAmount == null ? "" : data.m_Item1.SocialSecurityTaxWithholdingAmount.toFixed(2));
                    $("#txtW2MedicalWages").val(data.m_Item1.MedicareWagesAmount == null ? "" : data.m_Item1.MedicareWagesAmount.toFixed(2));
                    $("#txtW2MedicalWithheld").val(data.m_Item1.MedicareTaxWithholdingAmount == null ? "" : data.m_Item1.MedicareTaxWithholdingAmount.toFixed(2)),
                    $("#txtW2SSTips").val(data.m_Item1.SocialSecurityTipsAmount == null ? "" : data.m_Item1.SocialSecurityTipsAmount.toFixed(2)),
                    $("#txtW2AllocatedTips").val(data.m_Item1.AllocatedTipsAmount == null ? "" : data.m_Item1.AllocatedTipsAmount.toFixed(2)),
                    $("#txtW2DependentCare").val(data.m_Item1.DependentCareBenefitsAmount == null ? "" : data.m_Item1.DependentCareBenefitsAmount.toFixed(2)),
                    ...
                    //Yogalakshmi - 19th August 2014 - As Non Qualified plan is nullable field removed the null check condition.
                    $("#txtQualifiedPlan").val(data.m_Item1.NonQualifiedPlansAmount == null ? "" : data.m_Item1.NonQualifiedPlansAmount.toFixed(2)),
                    ...
                    if (data.m_Item1.HasEmployeeAddress != null && data.m_Item1.HasEmployeeAddress) {
                    ...
                    if (data.m_Item1.Employer != null && data.m_Item1.Employer.Organization != null) {
                        ...
                        $("#txtHouseholdEmployeeWages").val(data.m_Item1.Employer.HouseholdEmployeeWages == null ? "" : data.m_Item1.Employer.HouseholdEmployeeWages.toFixed(2));
                        if (data.m_Item1.Employer.Organization.Address.CompanyAddress.State != null)
                    ...
                    if (data.m_Item1.Employer != null && data.m_Item1.Employer.Organization != null) {
                        ...
                        $("#txtTaxableScholarshipAmount").val(data.m_Item1.Employer.TaxableScholarshipAmount == null ? "" : data.m_Item1.Employer.TaxableScholarshipAmount.toFixed(2));
                        if (data.m_Item1.Employer.Organization.Address.CompanyAddress.State != null)
