...
$(document).ready(function () {
    ...
    if (parameters.length == 3) {
        ...
        if (FormType.toString() == 'FormINT') {
            ...
            } //Saravanan N - 31st July, 2014 - Changes done to resolve the control focus issue. ELSE IF added.
            else if (controls.length == 1) {
                var controlToFocus = controls[0];
            }
            F1099INTGetData();

            //Saravanan N - 31st July, 2014 - Changes done to resolve the control focus issue. Moved out of IF condition.
    ...
    //Saravanan N - 19th Aug, 2014 - While loading the page itself, client side validation has to be displayed to the user.
    $('#txtpayerEIN').valid();
});
    ...
    //if ($("#frmInterestIncome").valid()) {
        ...
        if (f1099INT != null) {
            F1099INTPersistData = {
                ...
                //Yogalakshmi - 19th August 2014 - Changed from TaxExemptInterestStateId to TaxExemptInterestState which is the correct property to be binded.
                TaxExemptStateCodeAndAmount: f1099INT.TaxExemptStateCodeAndAmount,
                //TaxExemptInterestState: f1099INT.TaxExemptInterestState,
    ...
    //if ($("#frmInterestIncome").valid()) {
        ...
        if ((f1099OID != null) && (f1099OID.Payer != null || f1099OID.Owner != null || f1099OID.Form != null)) {
            F1099OIDPersistData = {
                ...
                ////Yogalakshmi - 19th August 2014 - Changed from TaxExemptInterestStateId to TaxExemptInterestState which is the correct property to be binded.
                //TaxExemptInterestState: f1099OID.TaxExemptInterestState,
                //StateExemptInterest: f1099OID.StateExemptInterest,
                TaxExemptStateCodeAndAmount: f1099OID.TaxExemptStateCodeAndAmount,
...
function setPopup() {
    $(function () {
        $("#popPayerName").popover({ title: 'Quick Help', html: 'true', content: "<h5>What is 'Payer Name'?</h5>It is the name of the Financial Institution or Bank who pays you the Interest Income.<br/><br/> Fill it in exactly as it appears on your Form 1099INT.<br/> <a href='' target='_blank' class='more'>Read Common Question</a>" });

        $("#popPayerEIN").popover({ title: 'Quick Help', html: 'true', content: "<h5>What is 'Payer FIN'?</h5>It is the Federal Identification Number (FIN) allotted to the Payer who provides you interest income.<br/><br/> Fill it in exactly as it appears on your Form 1099INT.<br/><a href='' target='_blank' class='more'>Read Common Question</a>" });
    });
}
