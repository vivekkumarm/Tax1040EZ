...
/* 20Aug2014 Vincent done changes on Accordian Focus Out persist and Error Message highlighting*/
...
$(document).ready(function () {
    ...
    $('#collapseOne').on('shown.bs.collapse', function () {
        if (!$('#taxesandwrapupinfo').valid()) {
            validateTaxesAndWrapFirstAccordion();
        }
    });

    $('#collapseTwo').on('shown.bs.collapse', function () {
        if (!$('#taxesandwrapupinfo').valid()) {
            validateTaxesAndWrapSecondAccordion();
        }
    });

    $('#collapseOne').on('hidden.bs.collapse', function () {
        validateTaxesAndWrapFirstAccordion();
        PersistTaxAndwrapup(false);
        return false;
    });
        ...
        success: function (data, textStatus) {
            if (data != null) {
                if (data.m_Item1 != null) {
                    ...
                    $("#txtFederalExtensionAmount").val(data.m_Item1.FederalExtensionAmount == null ? "" : data.m_Item1.FederalExtensionAmount.toFixed(2));
                    ...
                    //22Aug2014-Thileep Changed Code for Binding radio buttons
                    if (data.m_Item1.ThirdPartyDesignee != null && data.m_Item1.HasThirdPartyDesignee)
                        $('#radIsAllowOtherPersonYes').prop('checked', true);
                    else if (!data.m_Item1.HasThirdPartyDesignee && data.m_Item1.ThirdPartyDesignee == null)
                        $('#radIsAllowOtherPersonNo').prop('checked', true);

                    $("[name='radIsAllowOtherPerson']").trigger('change');
                    if (data.m_Item1.ThirdPartyDesignee != null && data.m_Item1.HasThirdPartyDesignee != null && data.m_Item1.HasThirdPartyDesignee) {
    ...
    //26Aug2014-Thileep Added Code for fix Radio button enlarges and aligned improper
    $('#radChecked').removeClass("form-control tip amountField").addClass("form-radio");
    $('#radSaved').removeClass("form-control tip amountField").addClass("form-radio");
    $('#radIsDirectDepositYes').removeClass("form-control tip amountField").addClass("form-radio");
    $('#radIsDirectDepositNo').removeClass("form-control tip amountField").addClass("form-radio");
    $('#radIsAllowOtherPersonYes').removeClass("form-control tip amountField").addClass("form-radio");
    $('#radIsAllowOtherPersonNo').removeClass("form-control tip amountField").addClass("form-radio");
...
function validateTaxesAndWrapFirstAccordion() {

    var validator = $("#taxesandwrapupinfo").validate();

    if (validator.invalid.txtFederalExtensionFiledDate == undefined && validator.invalid.txtDatePaid == undefined &&
        validator.invalid.txtDateOfFiling == undefined) {
...
function validateTaxesAndWrapSecondAccordion() {

    var validator = $("#taxesandwrapupinfo").validate();

    if (validator.invalid.txtPhoneNumber == undefined && validator.invalid.txtPhoneno == undefined &&
        validator.invalid.txtBankRoutingNumber == undefined && validator.invalid.txtBankAccountNumber == undefined) {
        //No error in this accordion.
