...
function Getaddressinfo() {
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ErrorHandling(jqXHR, errorThrown); //Saravanan N - 13th Aug, 2014 - Error handled in JS file.
        },
                ...
                if (data.Taxablestate != null) {
                    for (var i = 0; i < data.Taxablestate.length; i++) {
                        ...
                        if (data.Taxablestate[i].State != null)
                            bindList(data.Taxablestate[i].State.StateId, data.Taxablestate[i].State.StateName, data.Taxablestate[i].ResidentType, data.Taxablestate[i].IsReviewed);
                    }
...
function bindList(stateid, statename, residenttype, isreviewed) {
    var lblstatestarted = "Start";
    if (isreviewed) {
        lblstatestarted = "Review";
    }
    stateRecordCount += 1;
    var newresidenadress = ' <div class="pangird itm" id="divstateInformation' + stateRecordCount + '">' +
         ' <input type="hidden" id="hdnStateId' + stateRecordCount + '" value="' + stateid + '" />' +
          ...
          '<a href="#" title="' + lblstatestarted + '" id="lnkStartResidentState' + stateRecordCount + '" onclick="ReviewStates(' + stateid + ',\'' + statename + '\',\'' + residenttype + '\');return false;">' + lblstatestarted + '</a> ' +
        '   </label>' +
          ' </div>';
...
$(document).ready(function () {
    ...
    $("#stategateway").validate({
        ...
        messages: {
            ddlState: {
                required: "State Name is missing."
            }
        }
    });
    ...
    $('#btnAddstate').click(function () {
        ...
        if ($("#ddlState").val() == 0) {
            $("#divErrorMessage").show();
            $('#divErrorMessage').html('State Name is missing.');
            return false;
            ...
            if (isavailable) {
                ...
                var newresidenadress = ' <div class="pangird itm" id="divstateInformation' + stateRecordCount + '">' +
                      ...
                      '  <label id="lblStateName' + stateRecordCount + '">' + $("#ddlState option:selected").text() + '</label>' +
                       '   </label>' +
                      '<label class="col-md-4">' +
                       '  <label id="lblResident' + stateRecordCount + ' ">' + $("#ddlResidentType").val() + '</label>' +  //Partyearbindvalue 
                       '   </label>' +
                      '</label>' +
                        '<label class="col-md-4">' +
                     '<a href="#" title="Delete" id="lnkdeleteResidentState' + stateRecordCount + '" onclick="DeleteResidentState(this.id);return false;">Delete</a> ' +
                     '<a href="#" title="Start" id="lnkStartResidentState' + stateRecordCount + '" onclick="ReviewStates(' + $("#ddlState").val() + ',\'' + $("#ddlState option:selected").text() + '\',\'' + $("#ddlResidentType").val() + '\');return false;">Start</a> ' +
                     '   </label>' +
                      ' </div>';
...
function SaveStates(navigatetifinalsummary) {
...
function ReviewStates(stateid, statename, stateresidenttype) {
    if (stateid == '') {
        stateid = $('#hdnstaticstate').val();
        statename = $('#lblNameOfState').html();
        stateresidenttype = residentValue;
    }

    window.location.href = '../StateGateway/Residenttypeconfirmation.html?' + stateid + '-' + statename + '-' + stateresidenttype;
    return false;
