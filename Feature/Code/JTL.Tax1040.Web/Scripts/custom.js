...
//Added by Venkat S on 30 June 2014 for Pop Up issue.
 $(document).click(function (e) {
     ...
     $(".popover-title .close").remove();
     $('.popover-title').append('<button id="popoverclose" data-dismiss="popover"  type="button" class="close">&times;</button>');
     
     $('.help').each(function () {
         
         this.tabIndex = -1;
        
         if (!$(this).is(e.target) && $(this).has(e.target).length === 0 && $('.popover').has(e.target).length === 0 && $('.popoverclose').has(e.target).length == 0) {
