 ...
 * 05 August 2014 Swapnesh - Included Age Calculation for Personal and EIC
 * 07 August 2014 Swapnesh Encrypt the tax object string (Pass to Efile Service)
 * 21 August 2014 Bala EICAgeCalculation modified to allow the tax payer if DOB is greater than or equal to 1/1/1950 based on the disussion with BA 
---------------------
 */
using System;
...
namespace JTL.Tax1040.Core.Process
{
    public static class Utilities
    {
        ...
        /// <summary>
        /// Determines whether [is valid foreign phone] [the specified value].
        /// </summary>
        /// <param name="value">The value.</param>
        /// <returns></returns>
        public static bool IsValidForeignPhone(string value)
        {
            if (!string.IsNullOrEmpty(value))
            {
                ...
                if (IsNumeric(phonenumber))
                {
                    if (phonenumber == Constants.TwentyZeros || phonenumber == Constants.TwentyNines)
                    {
                        return false;
                    }
                    else
        ...
        /// <summary>
        /// Determines whether the SSN or ITIN no is valid.
        /// </summary>
        /// <param name="value">The value.</param>
        /// <returns>
        ///   <c>true</c> if the specified phone number is valid; otherwise, <c>false</c>.
        /// </returns>
        //Bala, 4 Sep 2014, Modified validation method name.
        public static bool IsValidSSNorITIN(string value)
        {
            if (!string.IsNullOrEmpty(value))
            {
                ...
                //Bala, 6Aug2014, Condition modified
                if (ssn.Length == Constants.SSNLength && IsNumeric(ssn) && ssn != Constants.NineZeros && ssn != Constants.NineNines)
                {
                    //4Aug2014, Bala, ignore the SSN validation If valid ITIN
                    if (!IsValidITIN(ssn))
                    {
                        ...
                        if (ssn == Constants.NaturalNumbers || areaCode == Constants.InvalidSsnAreaCode1 || areaCode1 == Constants.InvalidSsnAreaCode2 || groupCode == Constants.InvalidSsnGroupCode || serialNo == Constants.InvalidSsnSerialNo || areaCode > Constants.InvalidSsnAreaCodeRange)
                        {
                            return false;
                        }
                        else
        ...
        ///   <c>true</c> if the specified phone number is valid; otherwise, <c>false</c>.
        /// </returns>
        //Bala, 4 Sep 2014, added for SSN Validation
        ...
        /// </summary>
        /// <returns></returns>
        public static DateTime MinimumDOB()
        {
            return new DateTime(Constants.TAX_YEAR - 64, 01, 01);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static DateTime MaximumDOB()
        {
            return new DateTime(Constants.TAX_YEAR - 24, 01, 02);
        }
        ...
        /// <returns></returns>
        public static bool AgeCalculation(DateTime dateofbirth)
        {
            DateTime startdate = MinimumDOB();
        ...
        /// <param name="dateofbirth">The dateofbirth.</param>
        /// <returns></returns>
        public static bool EICAgeCalculation(DateTime dateofbirth)
        {
            DateTime startdate = MinimumDOB();
            DateTime endDate = MaximumDOB();
        ...
        #region Audit
        /// <summary>
        /// Persists the audit information.
        /// </summary>
        ...
        //Bala, 2-Sep-2014, Method added to persist Audit Info.
        public static void PersistAuditInfo(long userId, long userDataId, string description, string className, string tab, string topic, [CallerMemberName] string memberName = null)
        {
            var auditRepository = new AuditRepository();
            var auditInfo = new Audit
            {
                UserId = userId,
                UserDataId = userDataId,
                IPAddress = Utilities.GetIpAddress(),
                Description = string.Format(description, className, memberName),
                TabName = tab,
                TopicName = topic
            };

            auditRepository.Audit(auditInfo);
