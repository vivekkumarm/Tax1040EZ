 ...
 * 04July 2014 Thileep changed method AssertIsAccountNumber to avoid full zeros.
 * 17July 2014 Thileep changed method AssertIsAccountNumber.
 * 08July 2014 Bala Changed Special characters "_@./" removed from AssertIsNumbersLettersAndHyphen method.
 * 23July2014-Thileep added AssertIsPIN method to validate PIN number
 *  23July2014-Thileep added AssertIsAccountNumber method as per Spec Taxesanderapup(V1.5)
*/
using System;
...
namespace JTL.Tax1040.Core.Process
{
    public class BusinessValidationRepository : IBusinessValidationRepository
    {
        ...
        /// <param name="errorMessages">The error message collection</param>
        /// <returns></returns>
        public static ErrorMessage AssertIsValidEmail(string value, string errorCode, ErrorMessages errorMessages)
        {
            return (!Utilities.IsValidEmail(value) ? errorMessages[errorCode] : null);
        }


        /// <summary>
        /// Server side validation for date Formate 
        /// </summary>
        /// <param name="value"></param>
        /// <param name="errorCode"></param>
        /// <param name="errorMessages"></param>
        /// <returns></returns>
        /// vivek 8-July-2014 Added for the default validation
        public static ErrorMessage DateFormate(DateTime? value, string errorCode, ErrorMessages errorMessages)
        {
            return (!Utilities.IsValidRegexPattern(Convert.ToString(value), @"[^(0[1-9]|1[012])[- /.](0[1-9]|[12][0-9]|3[01])[- /.](19|20)\d\d+$")) ? errorMessages[errorCode] : null;
        }
        /// <summary>
        /// Validation used to determine if a given string is null
        /// and throws an exception if so.
            ...
            ErrorMessages errorMessages)
        {
            ...
            return (!Utilities.IsValidSSNorITIN(value)) ? errorMessages[errorCode] : null;
        }
        ...
        /// <summary>
        /// Asserts the is within foreign phone no.
        /// </summary>
        /// <param name="value">The value.</param>
        /// <param name="errorCode">The error code.</param>
        /// <param name="errorMessages">The error messages.</param>
        /// 19 Aug 2014-Vincent-Added a new method for Foreign phone number.
        /// <returns></returns>
        public static ErrorMessage AssertIsWithinForeignPhoneNo(string value, string errorCode, ErrorMessages errorMessages)
        {
            return (!Utilities.IsValidRegexPattern(value, @"[0-9]{1,20}") || (!Utilities.IsValidForeignPhone(value))) ? errorMessages[errorCode] : null;
        ...
        public static ErrorMessage AssertIsAlphapetOrNumber(string value, string errorCode,
                                       ErrorMessages errorMessages)
        {
            //Bala, 17Jul2014, Regex pattern modified to accept Alphanumeric only
            return (!Utilities.IsValidRegexPattern(value, @"[A-Za-z0-9]*$")) ? errorMessages[errorCode] : null;
        }
        ...
        /// <summary>
        /// Validation used to determine if a given string is Valid Account Number
        /// and throws an exception if so.
        /// </summary>
        /// <param name="value">The value.</param>
        /// <param name="name">Friendly parameter or property name</param>
        ...
        public static ErrorMessage AssertIsAccountNumber(string value, string errorCode, ErrorMessages errorMessages)
        {
            ...
            {
                long accValue = Utilities.ConvertToLong(value);
                return (accValue <= Constants.intZero || !Utilities.IsValidRegexPattern(value, @"[A-Za-z0-9-]{4,17}")) ? errorMessages[errorCode] : null;
            }
            else
            {
                return (!Utilities.IsValidRegexPattern(value, @"[A-Za-z0-9-]{4,17}")) ? errorMessages[errorCode] : null;
            }
        ...
        /// <summary>
        /// Validation used to determine if a given value is valid
        /// and throws an exception if so.
        /// </summary>
        /// <param name="value">The value.</param>
        /// <param name="name">Friendly parameter or property name</param>
        ...
        /// Error message if not valid
        /// </returns>       
        public static ErrorMessage AssertIsPIN(string value, int pinLengthValue, string errorCode,
            ErrorMessages errorMessages)
        {
            long accValue = Utilities.ConvertToLong(value);
            return (accValue <= Constants.intZero || value.Length != pinLengthValue) ? errorMessages[errorCode] : null;
        }
        ...
        /// <param name="errorMessages"></param>
        /// <returns></returns>
        /// vivek 9-july-14 Added the validation for the number only
        public static ErrorMessage AssertIsOnlyNumber(string value, string errorCode, ErrorMessages errorMessages)
        {
            int accValue = Utilities.ConvertToInteger32(value);
            return (accValue <= Constants.intZero || !Utilities.IsValidRegexPattern(value, @"[0-9]{1,5}")) ? errorMessages[errorCode] : null;
