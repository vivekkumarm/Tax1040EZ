    ...
    public class BussinessValidation
    {
        ...
        public void Validate()
        {
            try
            {
                ...
                List<BusinessMessagesDE> listBusinessMessagesDE = null;

                if (null != listIRSSchemaBE && 0 < listIRSSchemaBE.Count)
                {
                    foreach (IRSSchemaDE iRSSchemaBE in listIRSSchemaBE)
                    {
                        try
                        {
                            // For the picked up IRSSchema, Initially we set the State to 'Blank' &
                            // Stage to 'BVAL' in the EfileLastStatus table.
                            iRSSchemaBE.Status = 0;
                            iRSSchemaBE.State = State.Blank;
                            iRSSchemaBE.Stage = Stage.BusinessValidation;
                            businessValidationBL.UpdateSchemaState(iRSSchemaBE);


                            //  strFormType = iRSSchemaBE.FormType;

                            //getting information of return mode for testing 
                            Hashtable htSecurityInfo = businessValidationBL.GetSecurityInfo(strFormType);
                            ...
                            switch (strFormType)
                            {
                                case FormType.Form1040EZ:
                                    if (iRSSchemaBE.TaxYear == TaxYear.Year2009)
                                    {
                                        ...
                                        listBusinessMessagesDE = businessValidationTax1040EZ.BusinessValidations(iRSSchemaBE.SubmissionId,
                                                                 iRSSchemaBE.EfileIRSSchema, iRSSchemaBE.IRSSubmissionManifest, testMode);


                                        //BusinessValidation2009v2_0 businessValidation2009 = new BusinessValidation2009v2_0();
                                        //listBusinessMessagesDE = businessValidation2009.BusinessValidations(iRSSchemaBE.SubmissionId,
                                        //                         iRSSchemaBE.EfileIRSSchema, iRSSchemaBE.IRSSubmissionManifest, testMode);
                                    }
                                    else
                                    {
                                        ...
                                        //listBusinessMessagesDE = businessValidation70042010v2_1.BusinessValidations(iRSSchemaBE.SubmissionId,
                                        //                         iRSSchemaBE.EfileIRSSchema, iRSSchemaBE.IRSSubmissionManifest, testMode);
                                    }
                                    break;
                                ...
                                case FormType.Form8868:
                                    if (iRSSchemaBE.TaxYear == TaxYear.Year2009)
                                    {
                                        ...
                                        //listBusinessMessagesDE = businessValidation8868.BusinessValidations(iRSSchemaBE.SubmissionId,
                                        //                         iRSSchemaBE.EfileIRSSchema, iRSSchemaBE.IRSSubmissionManifest, testMode);
                                    }
                                    else
                                    {
                                ...
                                case FormType.Form4868:
                                    if (iRSSchemaBE.TaxYear == TaxYear.Year2009)
                                    {
                                        ...
                                        //listBusinessMessagesDE = businessValidation4868.BusinessValidations(iRSSchemaBE.SubmissionId,
                                        //                       iRSSchemaBE.EfileIRSSchema, iRSSchemaBE.IRSSubmissionManifest, testMode);
                                    }
                                    else
                                    {
                                ...
                                case FormType.Form2350:
                                    ...
                                    else
                                    {
                                        ...
                                        //listBusinessMessagesDE = businessValidation2350.BusinessValidations(iRSSchemaBE.SubmissionId,
                                        //                       iRSSchemaBE.EfileIRSSchema, iRSSchemaBE.IRSSubmissionManifest, testMode);
                                    }
                                    break;
                            ...
                            if (null != listBusinessMessagesDE && 0 < listBusinessMessagesDE.Count)
                            {
                                foreach (BusinessMessagesDE businessMessagesDE in listBusinessMessagesDE)
                                {
                                    ...
                                    businessMessagesDE.SubmissionID = iRSSchemaBE.SubmissionId;
                                    // businessValidationBL.SetErrorStatus(businessMessagesDE);
                                }

                                iRSSchemaBE.State = State.Error;
                            }
                            else
                            {
                                iRSSchemaBE.State = State.Success;
                            }
                            if (iRSSchemaBE.State == State.Error)
                            {
                                SendAlertMail("Business Validation Failed", iRSSchemaBE);
                            }
                            iRSSchemaBE.Status = 0;
                            iRSSchemaBE.Stage = Stage.BusinessValidation;
                            businessValidationBL.UpdateSchemaState(iRSSchemaBE);
