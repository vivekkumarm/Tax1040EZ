...
namespace JTL.Tax1040.EfileServices.ZipandSendService
{
    partial class IRSSubmissionZip : ServiceBase
    {
        ...
        public IRSSubmissionZip()
        {
            ...
            {
                if (!System.Diagnostics.EventLog.SourceExists("7004-ZIPP"))
                {
                    System.Diagnostics.EventLog.CreateEventSource("7004-ZIPP", "7004-ZIPP");
                }
                eventLog1.Source = "7004-ZIPP";
                eventLog1.Log = "7004-ZIPP";
            }
        ...
        /// </summary>
        public void GetEfileHoldID()
        {
            try
            {
                int.TryParse(ConfigurationManager.AppSettings["CommandTimeout"].ToString(), out cmdTimeout);

                List<long> eFileHoldIDs = iRSZipDAL.GetEfileHoldID(cmdTimeout);
                 
                foreach (long eFileHoldID in eFileHoldIDs)
                {
                    DataSet iRSSchemaDataSet = iRSZipDAL.GetIRSSchemaXml(eFileHoldID);
                    DoZipIRSXml(iRSSchemaDataSet, eFileHoldID);
                }
            }
            catch (Exception ex)
            {
        ...
        private void DoZipIRSXml(DataSet iRSSchemaDataSet, long eFileHoldID)
        {
            ...
            try
            {
                    ...
                    && 0 < iRSSchemaDataSet.Tables[0].Rows.Count)
                {
                    ...
                    {
                        iRSSchemaXml = iRSSchemaDataSet.Tables[0].Rows[0]["EfileIRSSchema"].ToString();
                        ...
                        manifestXml = iRSSchemaDataSet.Tables[0].Rows[0]["ManifestXML"].ToString(); 
                        manifestXml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + manifestXml;                                           

                        submissionId = iRSSchemaDataSet.Tables[0].Rows[0]["SubmissionID"].ToString();
        ...
        private void UpdateStatus(long efileHoldID, string submissionID, string state)
        {
            StatusBE statusBE = new StatusBE();
            statusBE.EfileHoldID = efileHoldID;
            statusBE.SubmissionId = submissionID;
            statusBE.Stage = Stage.Zip;
