    ...
    partial class IRSSender : ServiceBase
    {
        ...
        private bool LoadSecurityInfo()
        {
            try
            {
                ...
                Hashtable htSecurityInfo = objIRSSenderBL.GetSecurityInfo(FormType.Form7004);
        ...
        private void SendSubmission()
        {
            try
            {
                ...
                dsEfileSender = objIRSSenderBL.GetAllEfileSender(Stage.Zip, State.Success, cmdTimeout);
                    ...
                    if (Login())
                    {
                        ...
                        foreach (DataRow dr in dsEfileSender.Tables[0].Rows)
                        {
                            if (dr["DimeAttachment"] != null && dr["SubmissionID"] != null)
                            {
                                ...
                                objIRSSenderBL.SetStatus(dr["SubmissionID"].ToString(), Stage.SendSubmission, State.Blank);
                                Submit(dr);
                            }
                        }
                        LogOut();
                ...
                if (request.SubmissionsAttachmentMTOM != null)
                {
                    ...
                    if (response != null)
                    {
                        if (response.SubmissionReceiptListAttachmentMTOM != null)
                        {
                            ...
                            StoreSendSubmissionStatus(objIRSSenderDE, Stage.SendSubmission, State.Success);
                            IsSubmitted = true;
                        }
                    }
                }
        ...
        private void SetRequestResponseforSubmission(bool IsSubmitted)
        {
            try
            {
                ...
                IRSSenderBL objIRSSenderBL = new IRSSenderBL();
                objIRSSenderBL.SetStatus(objIRSSenderDE.SubmissionID, Stage.SendSubmission, strState);
        ...
        private void StoreSendSubmissionStatus(IRSSenderDE objIRSSenderDE, string strStage, string strState)
        {
            try
            {
                IRSSenderBL objIRSSenderBL = new IRSSenderBL();
                objIRSSenderBL.SetSendSubmissionStatus(objIRSSenderDE, strStage, strState);
            }
            catch (Exception ex)
            {
                WriteException(objIRSSenderDE.SubmissionID, ex);
