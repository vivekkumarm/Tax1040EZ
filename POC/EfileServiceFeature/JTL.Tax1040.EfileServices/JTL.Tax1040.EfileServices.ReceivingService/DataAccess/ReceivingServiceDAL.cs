...
namespace JTL.Tax1040.EfileServices.ReceivingService
{
    public class ReceivingServiceDAL : EfileDAL
    {
        ...
        public long PersistEfileReturnData(EfileReturnData eFileReturnData)
        {
            ...
            taxDB.AddInParameter(cmd, Parameters.StageId, DbType.Int64, JTL.Tax1040.EfileServices.Core.Process.Stage.ReceiveReturn);
            taxDB.AddInParameter(cmd, Parameters.State, DbType.String, "Success");
            taxDB.AddInParameter(cmd, Parameters.TaxYear, DbType.String, eFileReturnData.TaxYear);
            taxDB.AddInParameter(cmd, Parameters.NoOfAttachments, DbType.Int32, eFileReturnData.NoOfAttachments);



            taxDB.AddOutParameter(cmd, Parameters.Efile_return_data_id, DbType.Int64, 8);


            taxDB.ExecuteNonQuery(cmd);
            long efile_return_data_id = (long)taxDB.GetParameterValue(cmd, Parameters.Efile_return_data_id);
            return efile_return_data_id;
        }
