...
namespace JTL.Tax1040.EfileServices.ReceivingService
{
    public class TaxXMLtoIRSSchema
    {
        ...
        public IRSSchemaDE StartService(JTL.Tax1040.BusinessObject.Tax1040 UserData, long efile_return_data_id, bool finaltransmit)
        {
            ...
            try
            {
                ...
                if (GetSecurityInfo())
                {
                    ...
                    if (finaltransmit)
                    {
                        taxXMLtoIRSSchemaBL.statusUpdate(string.Empty, efile_return_data_id.ToString(), Stage.SchemaConversion, State.InProgress);
                    }
                    objIRSSchemaDE = ConverttoIRSSchema(UserData, efile_return_data_id, finaltransmit);
                    //                }
                    //            }
                    //        }
                }
        ...
        private bool GetSecurityInfo()
        {
            try
            {
                ...
                Hashtable htSecurityInfo = objTaxXMLtoIRSSchemaBL.GetSecurityInfo(FormType.Form1040EZ);
        ...
        private IRSSchemaDE ConverttoIRSSchema(JTL.Tax1040.BusinessObject.Tax1040 UserData, long efile_return_data_id, bool finaltransmit)
        {
            ...
            try
            {
                objIRSSchemaDE.EfileHoldID = Utilities.GetInt32(efile_return_data_id);
                objIRSSchemaDE.AttachmentsCount = "0";// UserData;  // dr["NoOfAttachments"].ToString();
                objIRSSchemaDE.Tax1040 = UserData;   // Swapnesh  objIRSSchemaDE.VendorXML = UserData;
                ...
                objIRSSchemaDE.FormType = FormType.Form1040EZ; //dr["TaxReturnTypeID"].ToString();

                //Check the form type and load the conversion logic
                switch (objIRSSchemaDE.FormType)
                {
                ...
                {
                    objIRSSchemaDE = GenerateIRSSubmissionManifest(objIRSSchemaDE, objIRSSchemaDE.FormType, EFIN);
                    if (objIRSSchemaDE.IsManifestGenerated)
                    {
                        objIRSSchemaDE.Status = Status.Converted1040EZXmlToIRSSchema;
                        if (finaltransmit)
                        {
                    ...
                    else
                    {
                        if (finaltransmit)
                        {
                            ...
                            taxXMLtoIRSSchemaBL.statusUpdate(string.Empty, objIRSSchemaDE.EfileHoldID.ToString(), Stage.SchemaConversion, State.Error);// dr["EfileHoldID"].ToString());
                        }
                    }
                }
                else
        ...
        //  Need to remove swapnesh
        private IRSSchemaDE ConverttoIRSSchema(DataRow dr)
        {
            //bool SuccessFullyConverted = false;
            IRSSchemaDE objIRSSchemaDE = new IRSSchemaDE();
            TaxXMLtoIRSSchemaBL taxXMLtoIRSSchemaBL = new TaxXMLtoIRSSchemaBL();
            try
            {
                ...
                if (objIRSSchemaDE.IsIRSSchemaConverted)
                {
                    ...
                    {
                        SendAlertMail("Manifest Generation Failed", objIRSSchemaDE);
                        taxXMLtoIRSSchemaBL.statusUpdate(string.Empty, dr["EfileHoldID"].ToString(), Stage.SchemaConversion, State.Error);
                    }
                }
                else
                {
        ...
        private void SetIRSShema(IRSSchemaDE objIRSSchemaDE, int strStage, string strState)
        {
            try
            {
                TaxXMLtoIRSSchemaBL objTaxXMLtoIRSSchemaBL = new TaxXMLtoIRSSchemaBL();
        ...
        private IRSSchemaDE GenerateIRSSubmissionManifest(IRSSchemaDE objIRSSchemaDE, string formType, string eFIN)
        {
            try
            {
                ...
                JTL.Tax1040.BusinessObject.Tax1040 objTaxEngineBO = objIRSSchemaDE.Tax1040;

                //JsonConvert.DeserializeObject<JTL.Tax1040.BusinessObject.Tax1040>(objIRSSchemaDE.VendorXML);
                ...
                else if (formType == FormType.Form1040A)
                {
                    Manifest7004(objIRSSubmissionManifest, objTaxEngineBO);
                }
                else if (formType == FormType.Form1040)
                {
                    Manifest1040EA(objIRSSubmissionManifest, objTaxEngineBO);
                }
        ...
        #region Manifest1040EA
        private static void Manifest1040EA(IRSSubmissionManifest objIRSSubmissionManifest, JTL.Tax1040.BusinessObject.Tax1040 objTaxEngineBO)
        {
            objIRSSubmissionManifest.TaxPeriodBeginDate = new DateTime(2014, 1, 1);
            objIRSSubmissionManifest.TaxPeriodEndDate = new DateTime(2014, 12, 31);
            objIRSSubmissionManifest.TaxPeriodEndDateSpecified = true;
            objIRSSubmissionManifest.TaxPeriodBeginDateSpecified = true;
            objIRSSubmissionManifest.TaxYear = "2014";
        }
        #endregion
