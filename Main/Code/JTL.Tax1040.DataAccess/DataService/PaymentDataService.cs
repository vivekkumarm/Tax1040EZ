...
namespace JTL.Tax1040.DataAccess
{
    public class PaymentDataService : IPaymentDataService
    {
        ...
        public double GetOrderAmount(PaymentDTO paymentDTO)
        {
            ...
            using (var sqlConnection = new SqlConnection(mTaxReturnDBConnectionString))
            {
                using (var sqlCommand = sqlConnection.CreateCommand())
                {
                    ...
                    // 07Jul2014 Sathish removed shoping cart id and set product id and user data id since order detail mapper is not clear
        ...
        public Order GetOrderWithOrderId(long orderID)
        {
            Order order = null;
            using (var sqlConnection = new SqlConnection(mTaxReturnDBConnectionString))
            {
                using (var sqlCommand = sqlConnection.CreateCommand())
                {
                    ...
                    using (var dataReader = sqlCommand.ExecuteReader())
                    {
                        if (dataReader.HasRows)
                        {
                            var userorderId = dataReader.GetOrdinal("order_id");
                            ...
                            var paymentDate = dataReader.GetOrdinal("time");
                            ...
                            while (dataReader.Read())
                            {
                                ...
                                {
                                    OrderId =
                                        (!dataReader.IsDBNull(userorderId)
                                             ? dataReader.GetInt64(userorderId)
                                             : 0L),
                                    TotalAmount =
                                    ...
                                    BillingDetails = new BillingDetails
                                    {
                                        FirstName = (!dataReader.IsDBNull(name)
                                             ...
                                             : ""),
                                        Email = (!dataReader.IsDBNull(email)
                                             ? dataReader.GetString(email)
                                             : ""),
                                        PaymentDate = (!dataReader.IsDBNull(paymentDate)
                                            ? dataReader.GetDateTime(paymentDate)
        ...
        {
            return GetOrderWithOrderId(paymentDTO.Order.OrderId);
        }
        ...
        public ShoppingCart GetUserFeatureSKU(PaymentDTO paymentDTO)
        {
            ...
            using (var sqlConnection = new SqlConnection(mTaxReturnDBConnectionString))
            {
                using (var sqlCommand = sqlConnection.CreateCommand())
                {
                    sqlConnection.Open();
                    sqlCommand.CommandType = CommandType.StoredProcedure;
                    sqlCommand.CommandText = "[dbo].[usp_get_user_feature_sku]";
                    sqlCommand.Parameters.AddWithValue("@product_id", (long)paymentDTO.ProductType);
                    sqlCommand.Parameters.AddWithValue("@user_data_id", paymentDTO.UserDataId);
                    ...
                    using (var dataReader = sqlCommand.ExecuteReader())
                    {
                        if (dataReader.HasRows)
                        {
                            var federalIdOrdinal = dataReader.GetOrdinal("federal_id");
                            var federalOrdinal = dataReader.GetOrdinal("federal");
                            var federalPriceOrdinal = dataReader.GetOrdinal("federal_price");

                            while (dataReader.Read())
                            {

                                FederalFeatureSKU federalFeatureSKU = new FederalFeatureSKU()
                                    {
                                        Feature = new Feature()
                                        {
                                            FeatureId = (!dataReader.IsDBNull(federalIdOrdinal)
                                                        ? dataReader.GetInt32(federalIdOrdinal)
                                                            : 0L),
                                            FeatureName = (!dataReader.IsDBNull(federalOrdinal)
                                                            ? dataReader.GetString(federalOrdinal)
                                                            : string.Empty)
                                        },
                                        FederalFeaturePrice = Convert.ToDouble(
                                                            (!dataReader.IsDBNull(federalPriceOrdinal)
                                                                 ? dataReader.GetDecimal(federalPriceOrdinal)
                                                                 : 0.0M))
                                    };
                                shoppingCart.FederalFeature.FederalFeatureSKU.Add(federalFeatureSKU);
                            }
                        }
                        ...
                        if (dataReader.HasRows)
                        {
                            ...
                            var statePriceOrdinal = dataReader.GetOrdinal("state_price");
                            ...
                            while (dataReader.Read())
                            {
                                StateFeature stateFeature = new StateFeature()
                                {
                                    ...
                                    State = new State()
                                    {
                                        StateId = (!dataReader.IsDBNull(stateIdOrdinal)
                                             ? dataReader.GetInt64(stateIdOrdinal)
                                             : 0L),
                                        StateName = (!dataReader.IsDBNull(stateNameOrdinal)
                                                    ? dataReader.GetString(stateNameOrdinal)
                                                    : string.Empty)
                                    }
                                };

                                StateFeatureSKU stateFeatureSKU = new StateFeatureSKU()
                                {
                                    Feature = new Feature()
                                    {
                                        FeatureId = (!dataReader.IsDBNull(stateIdOrdinal)
                                             ? dataReader.GetInt64(stateIdOrdinal)
                                             : 0L),
                                    },
                                    StatePrice = Convert.ToDouble((!dataReader.IsDBNull(statePriceOrdinal)
                                                                 ? dataReader.GetDecimal(statePriceOrdinal)
                                                                 : 0.0M)),
                                };


                                stateFeature.StateFeatureSKU.Add(stateFeatureSKU);

                                shoppingCart.StateFeature.Add(stateFeature);
                            }
                        }
        ...
        public bool IsOrderExistByUserDataId(PaymentDTO paymentDTO)
        {
            ...
            using (var sqlConnection = new SqlConnection(mTaxReturnDBConnectionString))
            {
                using (var sqlCommand = sqlConnection.CreateCommand())
                {
                    sqlConnection.Open();
                    sqlCommand.CommandType = CommandType.StoredProcedure;
                    sqlCommand.CommandText = "[dbo].[usp_is_order_exist_by_user_data_id]";
                    // 07Jul2014 Sathish removed shoping cart id and set product id and user data id since order detail mapper is not clear
                    sqlCommand.Parameters.AddWithValue("@product_id", paymentDTO.ProductType);
                    sqlCommand.Parameters.AddWithValue("@user_data_id", paymentDTO.UserDataId);

                    isOrderExist = Convert.ToBoolean(sqlCommand.ExecuteScalar());
                }
            }
            ...
            if (orders != null && orders.BillingDetails != null)
            {
                using (var sqlConnection = new SqlConnection(mTaxReturnDBConnectionString))
                {
                    using (var sqlCommand = sqlConnection.CreateCommand())
                    {
                        ...
                        sqlCommand.Parameters.AddWithValue("@user_data_id", userDataId); //21Jul2014 Sathish Passed user data id instead of user id
        ...
        public long CheckOutSummaryPersist(PaymentDTO paymentDTO)
        {
            ...
            using (var sqlConnection = new SqlConnection(mTaxReturnDBConnectionString))
            {
                using (var sqlCommand = sqlConnection.CreateCommand())
                {
                    ...
                    sqlCommand.Parameters.AddWithValue("@user_data_id", paymentDTO.UserDataId);
                    sqlCommand.Parameters.AddWithValue("@product_id", paymentDTO.ProductType);
                    // 07Jul2014 Sathish removed state ids
                    //sqlCommand.Parameters.AddWithValue("@state_ids", paymentDTO.StateIDs); 
        ...
        //15Jul2014 Sathish added get user feature sku
        public void PersistUserFeature(PaymentDTO paymentDTO)
        {
            using (var sqlConnection = new SqlConnection(mTaxReturnDBConnectionString))
            {
                using (var sqlCommand = sqlConnection.CreateCommand())
                {
                    sqlConnection.Open();
                    sqlCommand.CommandType = CommandType.StoredProcedure;
                    sqlCommand.CommandText = "[dbo].[usp_persist_user_feature]";
