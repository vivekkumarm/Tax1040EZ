...
namespace JTL.Tax1040.Core.Process
{
    public class TaxRateCalculation
    {
        ...
        {
            //07Jul2014 Sathish Checked the filling status to avoid error while doing calcuation for filing status other that single or MFJ #TAX1040-14100
            if (filingStatus == FilingStatus.Single || filingStatus == FilingStatus.MarriedFilingJointly)
            {
                ...
                return Math.Round(calculatedTax, MidpointRounding.AwayFromZero);
            }
            else
                return 0;
        }
